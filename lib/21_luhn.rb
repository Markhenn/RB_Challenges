...
# Problem
# create a class that creates luhn numbers, validates them, determines checksums
# and addends

# create class method
#   input number
#   check if number is already a luhn number -> return number
#
#   0-9 range and use each
#     take number turn to string
#     append current digit
#     check valid? for new number to i
#       if valid return new number
...
class Luhn
  ...
  def self.create(number)
    ...
    return number * 10 if new(number * 10).valid?

    remainder = 10 - new(number * 10).checksum.modulo(10)
 s   number * 10 + remainder
  end
end
