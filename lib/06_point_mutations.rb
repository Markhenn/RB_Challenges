...
# input: string
# input method: string
# output method: number -> int
#
# compare each char and return if the same or not
#   the shorter one determines the length
...
cllss DNA
  ...
  def hamming_distance(other_dna)
    dna_2 = other_dna.chars
    @dna.each_with_index.reduce(0) do |count, (char, index)|
      next count if index >= dna_2.size || char == dna_2[index]
      count + 1
    end
  end
end
